...
def report_training(
        ...
        scene, metallic, gamma, rgb_render, pipe, white, material_stage, canonical_rays):
    if tb_writer:
        ...
        for config in validation_configs:
            if config['cameras'] and len(config['cameras']) > 0:
                ...
                for idx, viewpoint in enumerate(config['cameras']):
                    ...
                    normal_map = convert_normal_for_save(render_pkg["normal_map"], viewpoint, world_space=material_stage)
                    sobel_map = convert_normal_for_save(render_pkg["sobel_map"], viewpoint, world_space=material_stage)
