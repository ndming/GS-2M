...
label = f'ours_wo-brdf_geo-2e-3_dn-0.03'
runtimes = []
for scene in scenes:
    scene_start = time.time()

    common_args = f"-r 2 --multi_view_geo_weight 2e-3 --lambda_depth_normal 0.03"
...
label = f'ours_wo-brdf_geo-1e-3_dn-0.03'
runtimes = []
for scene in scenes:
    scene_start = time.time()

    common_args = f"-r 2 --multi_view_geo_weight 1e-3 --lambda_depth_normal 0.03"
    cmd = f'python train.py -s {data_base_path}/scan{scene} -m {out_base_path}/scan{scene} {common_args}'
    print("[>] " + cmd)
    os.system(cmd)
