...
from .accessor import GaussianExportAccessor
from .base import ExportableModel, ModelExporter
from .transforms import estimate_normalizing_transform
from .serializer import (
    serialize_nurec_usd,
    serialize_usd_default_layer,
    write_to_usdz,
)
from .templates import NamedSerialized, fill_3dgut_template
...
class NuRecExporter(ModelExporter):
    ...
    def export(
        ...
        """
        print(f"[>] Exporting nurec usdz file to {output_path}...")
        ...
        if apply_transform and dataset is not None:
            try:
                ...
                print("[>] Applying normalizing transform to NuRec export")
            except (AttributeError, ValueError) as e:
                print(f"[>] Failed to apply normalizing transform: {e}")
                normalizing_transform = np.eye(4)
        ...
        if conf.render.method == "3dgut":
            ...
            print("[!] Warning: using 3DGUT NuRec template for 3DGRT data, may see slight loss of quality.")
