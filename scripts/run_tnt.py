...
label = f'ours_wo-brdf_abs-0.004'
runtimes = []
for scene in scenes:
    scene_start = time.time()

    common_args = f"-r 2 --densify_grad_abs_threshold 0.004"
...
label = f'ours_wo-brdf_abs-0.002'
runtimes = []
for scene in scenes:
    scene_start = time.time()

    common_args = f"-r 2 --densify_grad_abs_threshold 0.002"
    cmd = f'python train.py -s {data_base_path}/{scene} -m {out_base_path}/{scene} {common_args}'
    print("[>] " + cmd)
    os.system(cmd)
