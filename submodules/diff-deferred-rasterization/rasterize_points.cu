...
RasterizeGaussiansCUDA(
    ...
    torch::Tensor out_inv_depths = torch::full({1, H, W}, 0.0, float_opts);
    ...
    if (P != 0) {
        ...
        rendered = CudaRasterizer::Rasterizer::forward(
            ...
            out_inv_depths.contiguous().data_ptr<float>(),
            antialiasing,
            radii.contiguous().data_ptr<int>(),
            debug);
    }
    return std::make_tuple(rendered, out_colors, radii, geomBuffer, binningBuffer, imgBuffer, out_inv_depths);
}
...
RasterizeGaussiansBackwardCUDA(
        ...
        const bool debug) {
    const int P = means3D.size(0);
    const int H = grad_color.size(1);
    const int W = grad_color.size(2);
    ...
    float* dL_dinvdepths_ptr = nullptr;
    float* grad_inv_depth_ptr = nullptr;
    if (grad_inv_depth.size(0) != 0) {
        dL_dinvdepths = torch::zeros({P, 1}, means3D.options());
        dL_dinvdepths = dL_dinvdepths.contiguous();
        dL_dinvdepths_ptr = dL_dinvdepths.data_ptr<float>();
        grad_inv_depth_ptr = grad_inv_depth.data_ptr<float>();
    }
    ...
    if(P != 0) {
        CudaRasterizer::Rasterizer::backward(
            ...
            grad_color.contiguous().data_ptr<float>(),
            grad_inv_depth_ptr,
