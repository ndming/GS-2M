...
__global__ void preprocessCUDA(
    ...
    dL_dmean.x = (proj[0] * m_w - proj[3] * mul1) * dL_dmeans2D[idx].x + (proj[1] * m_w - proj[3] * mul2) * dL_dmeans2D[idx].y;
    dL_dmean.y = (proj[4] * m_w - proj[7] * mul1) * dL_dmeans2D[idx].x + (proj[5] * m_w - proj[7] * mul2) * dL_dmeans2D[idx].y;
    dL_dmean.z = (proj[8] * m_w - proj[11] * mul1) * dL_dmeans2D[idx].x + (proj[9] * m_w - proj[11] * mul2) * dL_dmeans2D[idx].y;
    ...
    // Otherwise, SHs were not used and gradients have been propagated to precomputed colors
    if (shs)
        computeColorFromSH(
            idx, D, M,
            (glm::vec3*)means3D,
    ...
    if (scales)
        computeCov3D(idx, scales[idx], scale_modifier, rotations[idx], dL_dcov3D, dL_dscales, dL_drots);
}
    ...
    for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE) {
        ...
        for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++) {
            ...
            for (int ch = 0; ch < featureCount; ch++) {
                ...
                const float dL_dchannel = dL_dbuffer[ch];
                dL_dalpha += (c - accum_buf[ch]) * dL_dchannel;
                atomicAdd(&(dL_dfeatures[global_id * F + ch]), dchannel_dcolor * dL_dchannel);
            }
            ...
            atomicAdd(&dL_dmeans2D[global_id].z, fabs(dL_dG * dG_ddelx * ddelx_dx));
            atomicAdd(&dL_dmeans2D[global_id].w, fabs(dL_dG * dG_ddely * ddely_dy));

            // Update gradients w.r.t. 2D covariance (2x2 matrix, symmetric)
            atomicAdd(&dL_dconics[global_id].x, -0.5f * gdx * d.x * dL_dG);
            atomicAdd(&dL_dconics[global_id].y, -0.5f * gdx * d.y * dL_dG);
            atomicAdd(&dL_dconics[global_id].w, -0.5f * gdy * d.y * dL_dG);
