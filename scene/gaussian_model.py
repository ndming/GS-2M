...
class GaussianModel:
    ...
    def get_normals(self, camera_center):
        ...
        min_axes = torch.zeros_like(scales).scatter(1, min_axix_idx, 1)
        # Pluck out columns of rotation matrices as normals
        rotations = build_rotation(self.get_rotation)
        normals = torch.bmm(rotations, min_axes.unsqueeze(-1)).squeeze(-1) # (N, 3)
    ...
    def create_from_pcd(self, pcd: BasicPointCloud, spatial_lr_scale: float):
        ...
        print(f"[>] Number of points at initialisation: {n_points}")
    ...
    def training_setup(self, training_args):
        ...
        self.radii2D_threshold = training_args.radii2D_threshold
        ...
        if training_args.prune_init_points:
            # Prune initial SfM points
            self.prune_init_points()
    ...
    def load_ply(self, path):
        ...
        params = (
            torch.tensor(xyz, dtype=torch.float, device="cuda"),
    ...
    def reduce_opacity(self):
        opacities_new = self.inverse_opacity_activation(torch.min(self.get_opacity, torch.ones_like(self.get_opacity) * 0.8))
        optimizable_tensors = self.replace_tensor_to_optimizer(opacities_new, "opacity")
        self._opacity = optimizable_tensors["opacity"]
    ...
    def prune_init_points(self):
        ...
        print(f"[>] Number of points pruned at setup: {sum(selected_pts_mask).item()}")
    ...
    def densify_and_split(self, grads, grad_threshold, grads_abs, grad_abs_threshold, scene_extent, max_radii2D, N=2):
        ...
        if selected_pts_mask.sum() + n_init_points > self.max_all_points:
            ...
            mask = (torch.max(self.get_scaling, dim=1).values > self.percent_dense * scene_extent) & (padded_max_radii2D > self.radii2D_threshold)
    ...
    def densify_and_prune(self, max_grad, max_grad_abs, min_opacity, extent, max_screen_size=None):
        ...
        self.densify_and_split(grads, max_grad, grads_abs, max_grad_abs, extent, self.max_radii2D.clone())

        # Remove transparent Gaussians
        prune_mask = (self.get_opacity < min_opacity).squeeze()
        if max_screen_size:
            # Remove Gaussians that are very large in worldspace and those that have a big footprint in viewspace
            # This helps control the number of Gaussians in the scene
    ...
    def add_densification_stats(self, viewspace_points, update_filter):
        # view-space point tensor contains view-space positional gradients
        self.xyz_gradient_accum[update_filter] += torch.norm(viewspace_points.grad[update_filter, :2], dim=-1, keepdim=True)
        self.xyz_gradient_accum_abs[update_filter] += torch.norm(viewspace_points.grad[update_filter, 2:], dim=-1, keepdim=True)
